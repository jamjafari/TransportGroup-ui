...
const ServiceDocumentsSection = ({ serviceId }) => {
  const { attachments, loading, addAttachment, removeAttachment } =
    useAttachments(ATTACHMENT_OWNER_TYPE.VEHICLE_SERVICE, serviceId);
  if (!serviceId) {
    return null;
  }
  ...
  return (
    <AppFormSection title="مدارک سرویس">
        ...
        {attachments.map((item) => (
          ...
          >
            <ListItemText
              ...
              secondary={`${categoryLabel(item.category)} — صدور: ${
                item.issueDate ? formatJalaliDate(item.issueDate) : '—'
              }`}
            />
          </ListItem>
        ))}
...
export default memo(ServiceDocumentsSection);
