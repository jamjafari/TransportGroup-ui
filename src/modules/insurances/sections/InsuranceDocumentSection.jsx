...
const InsuranceDocumentsSection = ({ insuranceId }) => {
  const { attachments, loading, addAttachment, removeAttachment } =
    useAttachments(ATTACHMENT_OWNER_TYPE.INSURANCE, insuranceId);
  if (!insuranceId) {
    return null;
  }
  ...
  return (
    <AppFormSection title="مدارک بیمه">
      <Grid container spacing={2} alignItems="flex-end">
        ...
        <Grid size={{ xs: 12, md: 3 }}>
          <AppJalaliDatePicker
            ...
            onChange={(value) =>
              setDraft((prev) => ({ ...prev, expiryDate: value }))
            }
          />
        </Grid>
        ...
        {attachments.map((item) => (
          ...
          >
            <ListItemText
              ...
              secondary={`${categoryLabel(item.category)} — انقضاء: ${
                item.expiryDate ? formatJalaliDate(item.expiryDate) : '—'
              }`}
            />
          </ListItem>
        ))}
...
export default memo(InsuranceDocumentsSection);
